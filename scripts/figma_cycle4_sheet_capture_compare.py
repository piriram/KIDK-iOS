...
SCREENS = [
    ScreenMap("06-city-selection", "6. 금액 가이드가 주어졌지만 직접 설정할 수 있는 화면.png", "mission-selection", 2.4),
    ScreenMap("07-city-creation", "7. 금액 가이드가 주어졌지만 직접 설정할 수 있는 화면.png", "mission-creation", 2.8),
]
...
def find_app_path() -> Path:
    override = os.environ.get("KIDK_APP_PATH")
    if override:
        path = Path(override).expanduser().resolve()
        if not path.exists():
            raise FileNotFoundError(f"KIDK_APP_PATH does not exist: {path}")
        return path
...
def main():
    if len(sys.argv) < 2:
        print("Usage: figma_cycle4_sheet_capture_compare.py <iter-name>")
        sys.exit(1)

    iteration = sys.argv[1]
    base_dir = REPO_ROOT / "docs/figma-compare/cycle4-sheet" / iteration
